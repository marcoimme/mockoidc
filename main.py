...
from config import settings, MOCK_USERS
from models import (
    TokenRequest, TokenResponse, DiscoveryResponse, 
    JWKSResponse, UserInfoResponse, AuthorizationCode
)
...
app = FastAPI(
    ...
    description="Mock OpenID Connect Provider for testing",
    version="1.0.0"
)
...
@app.get("/logout")
async def logout(
    post_logout_redirect_uri: Optional[str] = Query(default=None),
    state: Optional[str] = Query(default=None)
):
    """Logout endpoint"""
